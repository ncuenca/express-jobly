...
describe("GET /companies", function () {
  ...
  test("ok for anon with filter", async function () {
    ...
    const resp = await (request(app).get("/companies").send(query));
    expect(resp.body).toEqual({
      companies:
        [
          {
  ...
  test("Sad Path for anon w/ minEmployees over maxEmployees", async function () {
    const query = { minEmployees: 9000, maxEmployees: 10 }
    const resp = await (request(app).get("/companies").send(query));
  ...
  test("Sad Path for invalid inputs", async function () {
    const query = { name: 600, minEmployees: "fsdfd", maxEmployees: "ffdfd" }
    const resp = await (request(app).get("/companies").send(query));
